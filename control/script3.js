...
class Particle {
    ...
    startExplosion() {//function of explosion.
      let explosion = this.effect.explosions.find(exp => exp.free);
      if (explosion) {
...
class Effect {
    ...
    handleExplosions(context, deltaTime) {//new
...
function animate(time = 0) {
  ...
  ctx.clearRect(0, 0, canvas.width, canvas.height);
    ...
    effect.handleExplosions(ctx, deltaTime); // Pass the deltaTime for frame-based animation
    player.draw(ctx);
